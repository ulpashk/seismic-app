...
export default function Reinforcement({ buildings = [], isLoading = false }) {
  ...
  if (isLoading) {
    return (
      <div className="bg-[#d4edda] rounded-lg p-6">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">
          На основе индекса SRI, рекомендованные здания на усиление
        </h3>
  ...
  return (
    <div className="bg-[#d4edda] rounded-lg p-6">
      <h3 className="text-lg font-semibold mb-4 text-gray-800">
        На основе индекса SRI, рекомендованные здания на усиление ({sortedBuildings.length.toLocaleString()})
      </h3>
      ...
      <div className="bg-white rounded-md mb-3 p-3 shadow-sm">
        <div className="grid grid-cols-5 gap-2 text-xs font-medium text-gray-600">
          <div className="flex items-center gap-1">
            <span>Адрес</span>
            <ArrowUpDown className="w-3 h-3 cursor-pointer hover:text-green-600" onClick={() => handleSort("address")} />
          </div>
          <div className="flex items-center gap-1 justify-center">
            <span>SRI</span>
            <ArrowUpDown className="w-3 h-3 cursor-pointer hover:text-green-600" onClick={() => handleSort("sri")} />
          </div>
          <div className="flex items-center gap-1 justify-center">
            <span>H</span>
            <ArrowUpDown className="w-3 h-3 cursor-pointer hover:text-green-600" onClick={() => handleSort("h")} />
          </div>
          <div className="flex items-center gap-1 justify-center">
            <span>E</span>
            <ArrowUpDown className="w-3 h-3 cursor-pointer hover:text-green-600" onClick={() => handleSort("e")} />
          </div>
          <div className="flex items-center gap-1 justify-center">
            <span>V</span>
            <ArrowUpDown className="w-3 h-3 cursor-pointer hover:text-green-600" onClick={() => handleSort("v")} />
          </div>
        </div>
      </div>
