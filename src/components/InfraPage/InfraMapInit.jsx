...
export default function InfraMap({ selectedDistrict }) {
...
//   }, [districtFilter]);
    ...
    const urls = {
        readiness: "https://admin.smartalmaty.kz/api/v1/address/clickhouse/infra-readiness/",
        repgis: "https://admin.smartalmaty.kz/api/v1/address/clickhouse/rep-gis-infra/",
        social: "https://admin.smartalmaty.kz/api/v1/address/clickhouse/social-objects/",
    };

    if (!urls[mode]) return; // skip if not one of these modes

    const fetchData = async () => {
        try {
        const res = await fetch(urls[mode] + buildQuery());
        const data = await res.json();
        setGeoStructData(data);
        } catch (err) {
